...
var FILTERED_12S = ALL_ROWS_12S_DATA.slice();
var _byDrv12s = {};
var MODO_12S = false;
...
function setModo(is12s){
  ...
  if(leg)leg.innerHTML=is12s
    ?'<span class="pill pill-blue" style="font-size:9px">Sem infração</span><span class="pill cons-conversa" style="font-size:9px">Conversa</span><span class="pill cons-advert" style="font-size:9px">Advertência</span><span class="pill cons-recicla" style="font-size:9px">Reciclagem</span><span class="pill cons-rest3" style="font-size:9px">Susp. 30d</span><span class="pill cons-rest5" style="font-size:9px">Susp. 60d</span><span class="pill cons-comite" style="font-size:9px">Comitê/Afastar</span><span class="cc-badge" id="mat-count">—</span>'
  ...
  FILTERED=ALL_ROWS_PURO.slice();FILTERED_12S=ALL_ROWS_12S_DATA.slice();SEV_FILTER=null;
  applyFilters();
}
...
function applyFilters(){
  ...
  FILTERED=ALL_ROWS_PURO.filter(fn);
  FILTERED_12S=ALL_ROWS_12S_DATA.filter(fn);
  SEV_FILTER=null;
  updateFilterChips();
  renderAll();
}
...
function resetFilters(){
  ...
  FILTERED=ALL_ROWS_PURO.slice();FILTERED_12S=ALL_ROWS_12S_DATA.slice();SEV_FILTER=null;updateFilterChips();renderAll();
}
...
function getDriverCons(d){
  if(MODO_12S){
    var c=_byDrv12s[d.drv]||{b:0,mb:0,g:0,gv:0};
    if(!c.b&&!c.mb&&!c.g&&!c.gv)return 'Sem infração formal';
    var cats=(c.b>0?1:0)+(c.mb>0?1:0)+(c.g>0?1:0)+(c.gv>0?1:0);
    if(cats>=3)return 'Afastar + Comitê pela Vida';
    var s=c.gv>0?'GV':c.g>0?'G':c.mb>0?'M':'B';
    return getConsequencia(s,c.b+c.mb+c.g+c.gv);
  }
...
var _top10Drivers=[];var _top10Rows=[];var _top10Expanded=new Set();

function toggleTop10(idx){
  if(_top10Expanded.has(idx))_top10Expanded.delete(idx);else _top10Expanded.add(idx);
  renderTop10Inner();
}
function renderTop10Inner(){
  var tbody=document.getElementById('top10-tbody');
  if(!tbody)return;
  var maxEv=_top10Drivers[0]?_top10Drivers[0].ev:1;
  tbody.innerHTML=_top10Drivers.map(function(d,i){
    var pct=Math.round(d.ev/maxEv*100);
    var pillC=i<3?'pill-red':i<6?'pill-orange':'pill-yellow';
    var isOpen=_top10Expanded.has(i);
    var mainRow='<tr style="cursor:pointer" onclick="toggleTop10('+i+')">'
      ...
      +'<td style="font-weight:700;color:'+(d.maxV>=100?'var(--danger)':d.maxV>=90?'#ea580c':'var(--warn)')+'">'+d.maxV+' km/h</td>'
      +'<td style="color:var(--muted);font-size:10px">'+(isOpen?'▲ fechar':'▼ detalhes')+'</td>'
      +'</tr>';
    var detailRow='';
    if(isOpen){
      var dRows=_top10Rows.filter(function(r){return r[1]===d.drv;}).sort(function(a,b){return (b[2]+b[3]+b[4]+b[5])-(a[2]+a[3]+a[4]+a[5]);});
      var tblRows=dRows.map(function(r){
        var dt=r[0].slice(8,10)+'/'+r[0].slice(5,7)+'/'+r[0].slice(0,4);
        var ev=r[2]+r[3]+r[4]+r[5];
        var lim=r[8]||0;
        var pc=lim>0?Math.round((r[7]/lim-1)*100):0;
        var sv=pc>30?'Gravíssima':pc>20?'Grave':pc>10?'Média':'Baixa';
        var sc=pc>30?'#b91c1c':pc>20?'#c2410c':pc>10?'#a16207':'#15803d';
        var sb=pc>30?'#fee2e2':pc>20?'#ffedd5':pc>10?'#fef9c3':'#dcfce7';
        var dur=r[6]<60?r[6]+'s':Math.round(r[6]/60)+'min';
        return '<tr style="border-bottom:1px solid #e0e7ef">'
          ...
          +'<td style="padding:4px 10px;font-weight:700;color:'+sc+'">+'+(lim>0?pc+'%':'—')+'</td>'
          +'<td style="padding:4px 10px"><span style="background:'+sb+';color:'+sc+';padding:2px 7px;border-radius:20px;font-size:10px;font-weight:700">'+sv+'</span></td>'
          +'<td style="padding:4px 10px;color:var(--muted)">'+dur+'</td>'
          +'</tr>';
      }).join('');
      detailRow='<tr style="background:#f0f6ff"><td colspan="7" style="padding:10px 14px">'
        +'<div style="font-size:10px;font-weight:700;color:#1e40af;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Histórico completo — '+d.drv+'</div>'
        +'<div style="overflow-x:auto;max-height:280px;overflow-y:auto">'
        +'<table style="width:100%;border-collapse:collapse;font-size:11px">'
        +'<thead><tr style="background:#eff6ff">'
        +'<th style="padding:5px 10px;text-align:left;font-size:9px;text-transform:uppercase;letter-spacing:1px;color:#1d4ed8;white-space:nowrap;border-bottom:2px solid #bfdbfe">Data</th>'
        +'<th style="padding:5px 10px;text-align:center;font-size:9px;text-transform:uppercase;letter-spacing:1px;color:#1d4ed8;white-space:nowrap;border-bottom:2px solid #bfdbfe">Eventos</th>'
        +'<th style="padding:5px 10px;text-align:center;font-size:9px;text-transform:uppercase;letter-spacing:1px;color:#15803d;white-space:nowrap;border-bottom:2px solid #bfdbfe">Baixa</th>'
        +'<th style="padding:5px 10px;text-align:center;font-size:9px;text-transform:uppercase;letter-spacing:1px;color:#a16207;white-space:nowrap;border-bottom:2px solid #bfdbfe">Média</th>'
        +'<th style="padding:5px 10px;text-align:center;font-size:9px;text-transform:uppercase;letter-spacing:1px;color:#c2410c;white-space:nowrap;border-bottom:2px solid #bfdbfe">Grave</th>'
        +'<th style="padding:5px 10px;text-align:center;font-size:9px;text-transform:uppercase;letter-spacing:1px;color:#b91c1c;white-space:nowrap;border-bottom:2px solid #bfdbfe">Gravíssima</th>'
        +'<th style="padding:5px 10px;text-align:left;font-size:9px;text-transform:uppercase;letter-spacing:1px;color:#b91c1c;white-space:nowrap;border-bottom:2px solid #bfdbfe">Vel. Máx.</th>'
        +'<th style="padding:5px 10px;text-align:left;font-size:9px;text-transform:uppercase;letter-spacing:1px;color:var(--muted);white-space:nowrap;border-bottom:2px solid #bfdbfe">Limite</th>'
        +'<th style="padding:5px 10px;text-align:left;font-size:9px;text-transform:uppercase;letter-spacing:1px;color:#b91c1c;white-space:nowrap;border-bottom:2px solid #bfdbfe">Excesso</th>'
        +'<th style="padding:5px 10px;text-align:left;font-size:9px;text-transform:uppercase;letter-spacing:1px;color:var(--muted);white-space:nowrap;border-bottom:2px solid #bfdbfe">Severidade</th>'
        +'<th style="padding:5px 10px;text-align:left;font-size:9px;text-transform:uppercase;letter-spacing:1px;color:var(--muted);white-space:nowrap;border-bottom:2px solid #bfdbfe">Tempo</th>'
        +'</tr></thead>'
        +'<tbody>'+tblRows+'</tbody>'
        +'</table></div></td></tr>';
    }
...
// ── RENDER ALL ────────────────────────────────────────────────────
function renderAll(){
  // Monta lookup de contagens 12s por motorista (para cálculo de consequência)
  _byDrv12s={};
  if(MODO_12S){
    var r12=FILTERED_12S;
    if(SEV_FILTER){r12=r12.filter(function(r){var ev=0;if(SEV_FILTER.has('B'))ev+=r[2];if(SEV_FILTER.has('M'))ev+=r[3];if(SEV_FILTER.has('G'))ev+=r[4];if(SEV_FILTER.has('GV'))ev+=r[5];return ev>0;});}
    byDriver(r12).forEach(function(d){_byDrv12s[d.drv]={b:d.b,mb:d.mb,g:d.g,gv:d.gv};});
  }
  var rows = getRows(); // sempre de FILTERED (base completa para a matriz)
  // kpiRows: dados que alimentam KPIs, gráficos e ranking — 12s quando no modo 12s
  var kpiRows=MODO_12S?(SEV_FILTER?FILTERED_12S.filter(function(r){var ev=0;if(SEV_FILTER.has('B'))ev+=r[2];if(SEV_FILTER.has('M'))ev+=r[3];if(SEV_FILTER.has('G'))ev+=r[4];if(SEV_FILTER.has('GV'))ev+=r[5];return ev>0;}):FILTERED_12S):rows;
  var drvData=byDriver(rows).sort(function(a,b){return b.ev-a.ev;});
  var kpiDrvData=MODO_12S?byDriver(kpiRows).sort(function(a,b){return b.ev-a.ev;}):drvData;
  if(INACTIVE_DAYS>0){
    var _cut=new Date();_cut.setDate(_cut.getDate()-INACTIVE_DAYS);var _cutStr=_cut.toISOString().slice(0,10);
    drvData=drvData.filter(function(d){return d.lastDt>=_cutStr;});
    if(MODO_12S)kpiDrvData=kpiDrvData.filter(function(d){return d.lastDt>=_cutStr;});
  }
  // Em modo 12s a matriz exibe apenas motoristas com eventos >12s (mesmos do kpiDrvData)
  if(MODO_12S)drvData=kpiDrvData;
  var moData=byMonth(kpiRows);
  var totalEv=kpiDrvData.reduce(function(s,d){return s+d.ev;},0);
  var totalDur=(MODO_12S?FILTERED_12S:FILTERED).reduce(function(s,r){return s+r[6];},0);
  var totalB=kpiDrvData.reduce(function(s,d){return s+d.b;},0);
  var totalM=kpiDrvData.reduce(function(s,d){return s+d.mb;},0);
  var totalG=kpiDrvData.reduce(function(s,d){return s+d.g;},0);
  var totalGV=kpiDrvData.reduce(function(s,d){return s+d.gv;},0);
  var maxVel=(MODO_12S?FILTERED_12S:FILTERED).reduce(function(s,r){return Math.max(s,r[7]);},0);
  ...
  document.getElementById('k-drv').textContent=kpiDrvData.length;
  ...
  document.getElementById('sb-drv').textContent=kpiDrvData.length;
  ...
  // TOP 10
  _top10Drivers=kpiDrvData.slice(0,10);
  _top10Rows=kpiRows;
  _top10Expanded.clear();
  document.getElementById('top10-badge').textContent='Top '+Math.min(10,kpiDrvData.length)+' — '+periodLbl;
  renderTop10Inner();

  // EVOLUÇÃO TOP5 (clickable legend)
  var top5=kpiDrvData.slice(0,5).map(function(d){return d.drv;});
  var byDM={};
  kpiRows.forEach(function(r){var k=r[1]+'|'+r[0].slice(0,7);if(!byDM[k])byDM[k]=0;byDM[k]+=rowEv(r);});
...
fetch('/api/velocidade')
  ...
  .then(function(d){
    ...
    var p=d.payload;
    _RAW=p;ALL_ROWS_PURO=p.rows;ALL_ROWS=p.rows;
    if(p.rows12s&&p.rows12s.length>0)ALL_ROWS_12S_DATA=p.rows12s;
    ALL_DRIVERS=p.drivers;
    FILTERED=ALL_ROWS_PURO.slice();FILTERED_12S=ALL_ROWS_12S_DATA.slice();SEV_FILTER=null;expandedRows.clear();
    buildMs('ms-motorista',ALL_DRIVERS,'Todos os motoristas',applyFilters);
    applyFilters();
...
<div class="main">
  ...
  <div class="content">
    ...
    <div class="g-60-40">
      <div class="cc">
        <div class="cc-hdr"><div class="cc-title">Top 10 — maior número de desvios</div><div class="cc-badge" id="top10-badge">período selecionado</div><div style="font-size:10px;color:var(--muted)">clique em uma linha para ver os dias detalhados</div></div>
        <div class="tbl-wrap">
          <table>
            <thead><tr><th>#</th><th>Motorista</th><th>Eventos</th><th>Proporção</th><th>Tempo</th><th>Vel. Máx.</th><th></th></tr></thead>
